...
/*   Updated: 2023/05/03 17:48:39 by hkubo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char *argv[]) {
    ...
    // TODO: Support multiple servers by using thread
    for (unsigned int i = 0; i < servers.size(); i++) {
        std::stringstream ss;
        ss << servers[i]->get_port();
        char host_name[MAXLINE], port[MAXLINE];
        strcpy(host_name, servers[i]->get_host_name().c_str());
        strcpy(port, ss.str().c_str());
        int listen_fd;
        if (servers[i]->get_host_name() == "") {
        ...
        server.multiple_io_server_run(*(servers[i]));
    }
